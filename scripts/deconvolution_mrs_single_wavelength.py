...
print(f"Min data = {np.min(data[:,:,100,:])}, Max data = {print(np.max(data[:,:,100,:]))}")
...
plt.yscale("log")
print(f"Min data = {np.min(data[:,:,100,:])}, Max data = {np.max(data[:,:,100,:])}")
print(spectroModel.npix_slit_alpha_width)
...
plt.imshow(np.rot90(np.fliplr(res_fusion.x), -1))
plt.colorbar()
plt.show()
