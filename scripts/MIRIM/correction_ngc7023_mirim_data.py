...
mirim_save_data_dir = '/home/nmonnier/Data/JWST/NGC_7023/Fusion/Corrected_MIRIM/large_FoV/'
# filter_img = mirim_data_dir + 'Level3_F1000W_i2d_aligned.fits'

for filter_img in os.listdir(mirim_save_data_dir):
    # --- Load FITS ---
    with fits.open(mirim_save_data_dir + filter_img) as hdul:
    ...
    # -------- Sauvegarde FITS ----------
    def save_fits(output_path):
        header.add_history('Masked manually using RectangleSelector')
    ...
    def on_key(event):
        if event.key == 's':
            save_fits(mirim_save_data_dir + filter_img)
