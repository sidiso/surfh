...
def main():
    ...
    save_filter_corrected_dir = '/home/nmonnier/Data/JWST/small_NGC/Fusion/Filtered_slices/'
    ...
    for filename in sorted(os.listdir(save_corrected_dir)):  # iterates over all the files in 'path'
        ...
        with fits.open(save_corrected_dir + fits_name + '.fits') as hdul:
            ...
            NSlits = header['NSlits'] # Number of slits
            NWavel = header['NWavel'] # Number of wavelength points
            Nalpha = header['Nalpha'] # Number of alpha points
        ...
        header['NSlits'] = NSlits
        header['NWavel'] = NWavel
        header['Nalpha'] = Nalpha
        header['BAND'] = band
