...
from surfh.Signalprocessing import fitting, utilities
...
def process_pixel_chunk(chunk):
    ...
    for (i, j, raw_data_cube, peak_indices, mean_sigma, std_sigma) in chunk:
        ...
        fitted, continuum = fitting.fit_peaks_only(baseline_subtracted, spectrum, peak_indices, mean_sigma, std_sigma)
...
mad = utilities.sliding_mad(baseline_subtrated)

# Peak fitting
fitted_peaks = fitting.detect_and_fit_peaks(baseline_subtrated, mad, sigma=5, distance=5, wavelength=wavel)
...
axs[0].plot(wavel, np.nanmean(continuum_cube, axis=(1, 2)) + np.nanmean(spectral_line_cube, axis=(1, 2)), 
                                                            drawstyle='steps-mid', 
                                                            label='Separated Continuum', 
                                                            color='teal',
                                                            alpha = 0.75,
                                                            lw=2)
axs[0].plot(wavel, np.nanmean(continuum_cube, axis=(1, 2)), label='Input Spectrum', 
                                                            color='black', 
                                                            drawstyle='steps-mid', 
                                                            linestyle='dashed',
                                                            alpha=0.75,
                                                            lw=2)
axs[0].set_ylabel(r"Flux (MJy sr$^{-1}$)", fontsize=16)
axs[0].tick_params(axis='both', labelsize=14)  # taille des ticks
axs[0].legend(fontsize=14)

# --- Bottom panel: Gaussian fits over extracted lines ---
axs[1].plot(wavel, np.nanmean(raw_data_cube, axis=(1, 2)) - baseline, color='black', 
                                                                      linestyle='dashed', 
                                                                      drawstyle='steps-mid', 
                                                                      label='Separated lines', 
                                                                      alpha=0.75,
                                                                      lw=2)
axs[1].plot(wavel, np.nanmean(spectral_line_cube, axis=(1, 2)), color='teal', 
                                                                drawstyle='steps-mid',
                                                                label='Gaussians (best fit)', 
                                                                alpha=0.75,
                                                                lw=2)
axs[1].plot(wavel, np.nanmean(raw_data_cube, axis=(1, 2)) - baseline - np.nanmean(spectral_line_cube, axis=(1, 2)) - 5e2, 
                                                                color='crimson', 
                                                                linestyle='dotted', 
                                                                drawstyle='steps-mid', 
                                                                label='Residuals (shifted)', 
                                                                alpha=0.75,
                                                                lw=2)
axs[1].set_xlabel("Wavelength (microns)", fontsize=16)
axs[1].set_ylabel(r"Flux (MJy sr$^{-1}$)", fontsize=16)
axs[1].tick_params(axis='both', labelsize=14)
axs[1].legend(fontsize=14)

plt.tight_layout()
# plt.savefig(f'/home/nmonnier/Data/JWST/Simulation/Paper/Fusion/Plots/gaussian_fitting.png', dpi=300)
# plt.savefig(f'/home/nmonnier/Data/JWST/Simulation/Paper/Fusion/Plots/gaussian_fitting.pdf')
plt.show()
