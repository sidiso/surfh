...
alpha_axis = np.arange(maps.shape[1]) * step_Angle.degree
beta_axis = np.arange(maps.shape[2]) * step_Angle.degree
...
#%% Real data
path = '/home/nmonnier/Data/JWST/Orion_bar/ChannelCube_ch2-short_s3d.fits'
hdul = fits.open(path)
cube = hdul[1].data
...
of = 3 # Oversapling_factor -> To match the size of the model
...
of_ra_step_y = ra_step_y/3
ra_step_x = ra_map[6,5] - ra_map[5,5]
of_ra_step_x = ra_step_x/3

dec_step_y = dec_map[5,6] - dec_map[5,5]
of_dec_step_y = dec_step_y/3
dec_step_x = dec_map[6,5] - dec_map[5,5]
of_dec_step_x = dec_step_x/3
...
alpha_axis += hdul[1].header['CRVAL1']
beta_axis += hdul[1].header['CRVAL2']
xi = np.vstack(
    [
...
interpolated_im = griddata(points, TwoD_of_im, xi, method='linear').reshape(251,251)

plt.imshow(np.flipud(np.fliplr(interpolated_im.reshape(251,251))))
plt.show()
