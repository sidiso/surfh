...
class spectroSigRLSCT(LinOp):
    def __init__(
        ...
        oshape = (len(self.pointings), self.instr.n_slit, len(self.instr.wavel_axis), self.slicer.npix_slit_alpha_width)
    ...
    def forward(self, maps: np.ndarray) -> np.ndarray:
        ...
        for p_idx, pointing in enumerate(self.pointings):
            ...
            for slit_idx in range(self.instr.n_slit):
                ...
                allsliced[p_idx, slit_idx] = blurred_sliced_subsampled
        return allsliced
...
class spectroSigRLSCT_NN(LinOp):
    ...
    def adjoint(self, inarray: np.ndarray) -> np.ndarray:
        ...
        indexes_t = nearest_neighbor_interpolation.griddata((local_alpha_coord.ravel(), local_beta_coord.ravel()), local_cube[0].ravel(), (test_cube_alpha_axis.reshape(251,251), test_cube_beta_axis.reshape(251,251)))
        wavel_indexes_t = np.tile(indexes_t, (len(wavel_idx),1)).reshape(len(wavel_idx), len(indexes_t)) + ((wavel_idx[...,np.newaxis])*local_alpha_coord.shape[0]* local_alpha_coord.shape[1] )
        degridded = local_cube.ravel()[wavel_indexes_t].reshape(self.cube_shape)
