...
class NpDiff_r(aljabr.LinOp): # dim = 3
    ...
    def forward(self, x):
        Dx_masked = - np.diff(np.pad(x, ((1, 0), (0, 0)), 'wrap'), axis=0)
        return Dx_masked

    def adjoint(self, y):    
        Dy_masked = np.diff(np.pad(y, ((0, 1), (0, 0)), 'wrap'), axis=0)
        return Dy_masked
...
class NpDiff_c(aljabr.LinOp): # dim = 3
    ...
    def forward(self, x):
        Dx_masked = - np.diff(np.pad(x, ((0, 0), (1, 0)), 'wrap'), axis=1)        
        return Dx_masked   

    def adjoint(self, y):
        return np.diff(np.pad(y, ((0, 0), (0, 1)), 'wrap'), axis=1)
...
class Difference_Operator_Joint:  # gradients are joint
    ...
    def D(self, x):
        return irdftn(self.D_freq[np.newaxis, ...] * rdft2(x), shape=x.shape)

    def D_t(self, x):
        return irdftn(
            np.conj(self.D_freq[np.newaxis, ...]) * rdft2(x), shape=x.shape
        )

    def DtD(self, x):
        return irdftn(
            np.abs(self.D_freq[np.newaxis, ...]) ** 2 * rdft2(x), shape=x.shape
        )


#%%

class QuadCriterion_MRS_2D:
    ...
    def __init__(
        ...
        shape_target = model_spectro.imshape
        shape_of_output = model_spectro.ishape
        ...
        if type(self.mu_reg) == list or type(self.mu_reg) == np.ndarray:
            ...
            L_mu = self.mu_reg  # same mu for all maps
        self.L_mu = L_mu
