...
def reconstruction_MRS_fusion(MRSModel, ndata, templates, config, scale_data, data_dict, masks=None):
    ...
    Parameters:
        MRSModel: Spectro model object
        y_mrs: MRS Data array
        templates: Templates array
        config: Configuration object
        scale_data: Scale data parameter
        data_dict: Data dictionary
        masks: Optional masks for saving results
    """
    value_init = 0
...
def reconstruction_MIRIM_fusion(MIRIMModel, y_mirim, templates, config, scale_data):
    """
    Perform the reconstruction method and save results.
    ...
    if templates is None:
        ...
        shape_templates = templates.shape[0]
    result_dir = f'MIRIM_{config.reconstruction.method}_Temp_{shape_templates}_nit_{str(config.reconstruction.max_iter)}_mu_{float(config.reconstruction.mu):.2e}_SD_{scale_data}/'
    print(f"Result path = {config.configuration.result_dir }")
    print(f"Result dir = {result_dir }")
    ...
    quadCrit_fusion = QuadCriterion_MRS(
        mu_spectro=1,
        y_spectro=np.copy(y_mirim),
        model_spectro=MIRIMModel,
        mu_reg=config.reconstruction.mu,
        printing=True,
        gradient="separated"
    )
    ...
    if templates is None:
        print("No templates")
        print(f"Results save in {path}")
        save_mirim_to_fits(np.array(res_fusion.x), path /'res_cube.fits')
        np.save(path / 'criterion.npy', quadCrit_fusion.L_crit_val)
    else:
        # Convert maps to cube
...
def reconstruction_MIRIM_MRS_method(MRSModel, y_mrs, MIRIMModel, y_mirim, templates, config, scale_data, data_dict, masks=None):
    ...
    Parameters:
        MRSModel: Spectro model object
        y_mrs: MRS Data array
        MIRIMModel: Imager model object
        y_mirim: Imager data array
        templates: Templates array
        config: Configuration object
        scale_data: Scale data parameter
        data_dict: Data dictionary
    ...
    result_dir = f'MIRIM_MRS_{config.reconstruction.method}_MC_{len(MRSModel.instrs)}_MO_4_Temp_{shape_templates}_nit_{str(config.reconstruction.max_iter)}_mu_{float(config.reconstruction.mu):.2e}_SD_{scale_data}/'
    print(f"Result path = {config.configuration.result_dir }")
    print(f"Result dir = {result_dir }")
    ...
    # Run the method
    res_fusion = quadCrit_fusion.run_lcg(config.reconstruction.max_iter, perf_crit=1, calc_crit=True, value_init=value_init)
