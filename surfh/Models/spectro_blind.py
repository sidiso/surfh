...
class MRSBlurred(LinOp):
        ...
        for p_idx, pointing in enumerate(self.pointings):
            ...
            for slit_idx in range(self.instr.n_slit):
                oversampled_sliced = np.repeat(
                        ...
                        self.npix_slit_beta_width,
                        axis=1,
                    )/(self.npix_slit_beta_width * self.srf)
                blurred_t_sliced = np.zeros(self.get_slit_shape_t())
                blurred_t_sliced[: self.slices_shape[2] * self.srf : self.srf,:] = oversampled_sliced
                local_img += self.slicing_t(blurred_t_sliced, slit_idx, self.local_im_shape)
    ...
    def gridding(self, blurred_img: array, pointing: instru.Coord) -> array:
        ...
        blurred_img = blurred_img[np.newaxis, ...]
        gridded = cython_utils.interpn_cube2local(np.arange(1).astype(np.float64), 
                                                   self.alpha_axis, 
                                                   self.beta_axis, 
                                                   np.array(blurred_img).astype(np.float64), 
                                                   optimized_local_coords, 
                                                   (1, len(self.local_alpha_axis), len(self.local_beta_axis)))
        return gridded[0]
    ...
    def gridding_t(self, local_img: array, pointing: instru.Coord) -> array:
        ...
        global_img = cython_utils.interpn_local2cube(np.arange(1), 
                                                self.local_alpha_axis.ravel(), 
                                                self.local_beta_axis.ravel(), 
                                                np.array(local_img, dtype=np.float64), 
                                                optimized_global_coords, 
                                                (len(np.arange(1)), len(self.alpha_axis), len(self.beta_axis)))
        return global_img[0]
