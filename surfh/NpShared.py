...
def CreateShared(Name, shape, dtype):
    try:
        ...
        print("File %s exists, deleting" % Name)
        DelArray(str(Name))
        a = SharedArray.create(str(Name), shape, dtype=dtype)
    return a
...
def SharedToDico(Prefix):
    ...
    print("SharedToDico: start [prefix = %s]" % Prefix)
    Lnames = ListNames()
    ...
    for Sharedkey in keys:
        ...
        print("  %s -> %s" % (Sharedkey, key))
        Shared = GiveArray(Sharedkey)
        if isinstance(Shared, type(None)):
            print("      None existing key %s" % (key))
            return None
        DicoOut[key] = Shared
    print("SharedToDico: done")
    return DicoOut
